...
- `constants/groups.ts` — group types, `canCoachGroup(role)`, and the group-chat key helpers. Since migration 0027 a `group_members` row means **invited**; `accepted_at` is what makes it a membership, and every "is this person in this group?" question routes through the database's `is_group_member()`, so a pending invitee is out of the thread, the roster and the read stamps without any screen having to check. `GroupInvite` is deliberately thin — it comes from `get_my_group_invites()`, not from the group row, and is the whole of what someone may know about a group they haven't joined. A group program send still goes to EVERY invited member so it's waiting for them on accept.
...
| Member → group ("For Review") | `review` | **one** | the group's owner | any coach reviews, sends back, and marks done |

A review is one row on purpose: it's a queue item, not a request for N second opinions, and N rows would mean N returned snapshots of one program with nothing to say which the member should follow. `recipient_id` is the owner only because the column is `NOT NULL` — `coaches_group()` decides who may act. That's why `loadGroupReviewPrograms(groupId)` exists: `loadSentPrograms` would only ever surface it to the owner, so a trainer opening it from the group page passes `groupId` through to the review screen.

`loadSentPrograms` deliberately hides group reviews from a TRAINER's inbox (`!r.group_id`) and keeps them in the SENDER's list. The group's backlog belongs to the group, not to whoever owns it; a program the member posted is still one they sent. Clearing the queue is `completed_at`, written only through `set_group_review_completed` so a sender can't close their own review — it drops the item from the group and leaves the member's copy untouched.
