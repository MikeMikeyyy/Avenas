...
- `utils/workout.ts` — the **single source of truth** for resolving an active program's scheduled workout and for "previous values". `getTodaysWorkout(program)`, `getWorkoutForDate(program, "YYYY-MM-DD")` and `resolveTodayWorkout(program, override)` (honors a same-day change-day override) return `{ dayIndex, name, exercises } | null` via the documented cycle math; `resolveDayIndex(...)` exposes just the index. `buildPrevByName(history, beforeDate?, dayName?)` builds the last-set-per-exercise map keyed by `normalizeExerciseName(name)` (trim + lowercase); pass the current workout-day name as `dayName` so an exercise programmed on two days gets that day's last numbers (same-day sessions win; any-day is only a fallback for exercises never done on the day). Screens MUST call these instead of re-implementing the cycle math or prev-set lookup inline.
...
| `@avenas/workout_history`                      | `CompletedWorkout[]` — always prepended, so *mostly* newest-first; a backdated journal log is prepended too, so consumers sort by `completedAt` rather than trusting order (they all do). | workout, log-workout, workout (delete-redo) |
| `@avenas/workout_dates`                        | `string[]` of `"YYYY-MM-DD"`, deduped.                                                                                          | workout, log-workout                       |
| `@avenas/today_workout_override`               | `{ date: "YYYY-MM-DD"; workoutName: string; programId?: string }` — honored only when `date` matches the effective day. `programId` = the chosen day's source program (change-day can pick from a non-active program); absent on free-workout overrides and legacy records. | workout                                    |
| `@avenas/workout_draft`                        | `{ date, workoutInfo, log, isometricExIds, notes, isFreeWorkout, freeWorkoutAddToProgram }` — **single global** in-progress draft. The notes text persists here; `sessionNotes` always commits whatever text was written at Finish (clearing the text drops the note). The session-notes card is a floating overlay opened from the bottom-left action cluster; its open/closed state (`showNotes`) is transient and not persisted. | workout                                    |
...
| `@avenas/cycle_pattern_coachmark_seen`         | `"1"` — one-shot flag: the builder's tap-a-day coach mark has been shown. Local-only.                                           | new-program                                |
...
`@avenas/today_workout_override` stores `{ date, workoutName, programId? }`. Honor it only when `override.date` matches the effective day — stale overrides from a previous day must be ignored. Resolution goes through `resolveWorkoutForDate(program, override, dateYMD, allPrograms?)`: the day is looked up in its **source** program (`programId`, which change-day records because the user can pick a day from a non-active program), falling back to the active program when `programId` is absent (legacy override / free workout) or its program is gone. Pass the full `allPrograms` list or cross-program overrides degrade to name-only. The override key is also used by the "free workout" flow to remember the user-chosen name across reloads (no `programId`).
